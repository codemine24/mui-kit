...
export function StripedColumnTablePreview() {
  const theme = useTheme();

  const getColumnBgColor = (index: number) =>
    index % 2 === 0 ? theme.palette.primary.main : theme.palette.info.main;
          ...
          <TableHead>
            <TableRow>
              {headers.map((header, index) => (
                <TableCell key={header} sx={{ backgroundColor: getColumnBgColor(index) }}>
            ...
            {salesData.map((row, rowIndex) => (
              <TableRow key={rowIndex}>
                {[row.product, row.jan, row.feb, row.mar].map((value, colIdx) => (
                  <TableCell key={colIdx} sx={{ backgroundColor: getColumnBgColor(colIdx) }}>
                    {value}
                  </TableCell>
                ))}
              </TableRow>
