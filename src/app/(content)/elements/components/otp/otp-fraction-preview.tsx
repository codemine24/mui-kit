...
export const OTPFractionPreview = () => {
  const [firstGroup, setFirstGroup] = useState<string[]>(new Array(firstLength).fill(""));
  const [secondGroup, setSecondGroup] = useState<string[]>(new Array(secondLength).fill(""));
  ...
  const handleFirstGroupChange = (element: HTMLInputElement, index: number) => {
    ...
    const newFirstGroup = [...firstGroup.map((d, idx) => (idx === index ? element.value : d))];
  ...
  const handleSecondGroupChange = (element: HTMLInputElement, index: number) => {
    if (isNaN(Number(element.value))) return false;

    const newSecondGroup = [...secondGroup.map((d, idx) => (idx === index ? element.value : d))];
  ...
  const handleFirstGroupKeyDown = (e: React.KeyboardEvent<HTMLDivElement>, index: number) => {
    if (e.key === "Backspace" && !firstGroup[index] && index > 0) {
      firstGroupRefs.current[index - 1]?.focus();
    }
  };

  const handleSecondGroupKeyDown = (e: React.KeyboardEvent<HTMLDivElement>, index: number) => {
  ...
  const handlePaste = (e: React.ClipboardEvent, isFirstGroup: boolean, index: number) => {
    e.preventDefault();
    ...
    if (pasteData.length === firstLength + secondLength) {
      ...
      const secondPart = pasteData.slice(firstLength, firstLength + secondLength).split("");
  ...
  return (
    <Box>
      <Box display="flex" alignItems="center" gap={{ xs: 1, md: 2 }}>
        ...
        <Box display="flex" gap={1}>
          {firstGroup.map((data, index) => (
            <TextField
              ...
              value={data}
              onChange={(e) => handleFirstGroupChange(e.target as HTMLInputElement, index)}
        ...
        {/* Separator */}
        <Typography variant="h4" sx={{ color: "text.secondary", fontWeight: "bold", mx: 1 }}>
          -
        </Typography>
        ...
        <Box display="flex" gap={1}>
          {secondGroup.map((data, index) => (
            <TextField
              ...
              value={data}
              onChange={(e) => handleSecondGroupChange(e.target as HTMLInputElement, index)}
