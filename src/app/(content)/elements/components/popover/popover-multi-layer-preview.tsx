          ...
          <Paper elevation={3}>
            <MenuList>
              {item.subItems?.map((subItem, index) => (
                <MenuItemWithArrow key={index}>{subItem.label}</MenuItemWithArrow>
              ))}
            </MenuList>
          </Paper>
        </Grow>
...
export const PopoverMultiLayerPreview = () => {
  ...
  const menuItems: MenuItemType[] = [
    {
      label: "Electronics",
      subItems: [{ label: "Computers" }, { label: "Phones" }]
    },
    {
      label: "Clothing",
      subItems: [{ label: "Men" }, { label: "Women" }]
    },
    {
      label: "Women",
      subItems: [{ label: "Dresses" }, { label: "Tops" }, { label: "Accessories" }]
    },
    { label: "Books" },
  ...
  return (
    <>
          ...
          <Paper>
            <MenuList>
              {menuItems.map((item, index) => (
                <MenuItemWithArrow key={index} onMouseEnter={(e) => handleMouseEnter(e, item)}>
                  {item.label}
      ...
      {hoveredItem && hoveredItem.subItems && (
        <SubMenu item={hoveredItem} parentEl={hoverAnchorEl} />
      )}
    </>
  );
};
