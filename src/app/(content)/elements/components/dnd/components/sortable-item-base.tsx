  ...
  return (
    <Box
      ...
      sx={{
        flexShrink: 0,
        display: "flex",
        transform: `translate3d(${Math.round(stateProps?.transform?.x || 0)}px, ${Math.round(
          stateProps?.transform?.y || 0
        )}px, 0) scaleX(${stateProps?.transform?.scaleX ?? 1}) scaleY(${
          stateProps?.transform?.scaleY ?? 1
        })`,
        transformOrigin: "0 0",
